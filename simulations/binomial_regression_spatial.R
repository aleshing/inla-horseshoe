...
dat <- sim_binomial_regression_spatial(loc, p=10, num_zero=3, n_trial=25,
                                       sre.sigma=1, sre.rho=0.5, seed=72)
plot(dat$loc)
...
time <- system.time(
    results <- INLAMH(d=dat, fit.inla=hs_binomial_regression_spatial_inla,
...
plot_INLA_hyperpar(dat, marginals, spatial=TRUE, layout=c(1, 2))

# Save full results
save(dat, bound, mesh, A, iter, scale, time, results, accepts, samps, marginals,
     file="results/binomial_regression_spatial_results_full.RData")

# Save only processed results
save(dat, bound, mesh, A, iter, scale, time, accepts, samps, marginals,
     file="results/binomial_regression_spatial_results_processed.RData")

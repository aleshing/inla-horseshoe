...
time <- system.time(
    results <- INLAMH(d=dat, fit.inla=hs_binomial_regression_inla,
...
marginals <- process_INLA(dat, iter, results$model.sim, hyper=FALSE)
...
save(dat, iter, scale, time, results, accepts, samps, marginals,
     file="results/binomial_regression_results_full.RData")

# Save only processed results
save(dat, iter, scale, time, accepts, samps, marginals, 
     file="results/binomial_regression_results_processed.RData")
